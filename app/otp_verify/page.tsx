...
function OtpVerifyContent() {
  ...
  }, [email, resendTimer, resendLoading])
      ...
      if (result.status === true) {
        const message =
          typeof result.message === 'string'
            ? result.message
            : 'Email verified successfully. Your account has been created and you can now log in.'
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: '#FDFBF0' }}>
      <header className="flex items-center justify-between px-6 sm:px-10 py-4">
        ...
        <Link href="/login" className="flex items-center no-underline">
          <span
      ...
      <div className="flex items-center justify-center px-4 py-10 sm:py-16">
        <div
          className="w-full max-w-lg rounded-2xl border px-8 py-10 sm:px-12 sm:py-12"
          style={{ borderColor: '#D6D1C4', backgroundColor: '#FDFBF0' }}
        >
          <div className="text-center mb-8">
            <div
              className="mx-auto mb-5 flex h-14 w-14 items-center justify-center rounded-full"
              style={{ backgroundColor: '#E8D4F8' }}
            >
            ...
            <h1
              ...
              style={{ color: '#054742', fontFamily: 'Georgia, "Times New Roman", serif' }}
            >
              Verify Your Email
            </h1>
            <p className="text-sm" style={{ color: '#6B6B6B' }}>
              {email ? (
                <>
                  ...
                  <span className="font-medium" style={{ color: '#054742' }}>
                    {maskEmail(email)}
                  </span>
                </>
              ) : (
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <label className="block text-sm font-medium mb-3 text-center" style={{ color: '#054742' }}>
                Verification Code
              </label>
              <div className="flex justify-center gap-2 sm:gap-3" onPaste={handlePaste}>
                {otp.map((digit, index) => (
                  <input
                    ...
                    className="h-12 w-10 sm:h-14 sm:w-12 rounded-lg border text-center text-lg font-semibold outline-none transition-colors focus:ring-2"
                    style={{
                      borderColor: error ? '#C75050' : '#D6D1C4',
              ...
              {error && (
                <p className="mt-3 text-center text-xs" style={{ color: '#C75050' }}>
                  {error}
                </p>
              )}
              {successMessage && (
                <p
                  className="mt-3 text-center text-xs leading-relaxed rounded-lg px-3 py-2.5"
                  style={{ color: '#054742', backgroundColor: '#E8F5E9', border: '1px solid #B8D4BB' }}
                >
                  {successMessage}
                </p>
            ...
            <button
              ...
              className="w-full py-3.5 rounded-lg text-white font-semibold text-sm transition-all hover:opacity-90 disabled:opacity-60 disabled:cursor-not-allowed"
              style={{ backgroundColor: '#B48AE0' }}
            >
              {loading ? 'Verifying...' : successMessage ? 'Redirecting to login...' : 'Verify & Continue'}
            </button>
          </form>
          ...
          <p className="mt-6 text-center text-sm" style={{ color: '#6B6B6B' }}>
            ...
            {resendTimer > 0 ? (
              <span style={{ color: '#9B9B9B' }}>Resend in {resendTimer}s</span>
            ) : (
              <button
                type="button"
                onClick={() => void handleResend()}
                disabled={resendLoading}
                className="font-medium underline transition-opacity hover:opacity-80 disabled:opacity-60 disabled:cursor-not-allowed"
                style={{ color: '#054742' }}
              >
          ...
          <p className="mt-4 text-center text-xs" style={{ color: '#9B9B9B' }}>
            Wrong email?{' '}
            <Link href="/signup" className="underline" style={{ color: '#054742' }}>
              Go back to sign up
            </Link>
          </p>
        </div>
